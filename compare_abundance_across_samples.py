...
def main(cm_unique_peps_csv, cm_dict, huvec_unique_peps_csv, huvec_dict, scaled):
    log_dir = os.path.dirname(cm_unique_peps_csv)

    log_file = open(os.path.join(
        log_dir, f'test_{scaled}_differences_in_exp_unique_peptides.out'), 'w')

    # Redirect only stdout (used by print), but leave stderr alone
    sys.stdout = log_file
    ...
    ################################################################################
    print(
        f'Test differences in {scaled} expression of unique peptides for CM NMD vs DMSO')
    wilcoxon_signed_test_ps = []
    for unique_peptide in cm_unique_peptdies_df.index:

        wilcox_signed_result = stats.wilcoxon(
            x=np.array(
                cm_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 130C Sample',
                                                           f'Abundances {scaled} F1 131N Sample',
                                                           f'Abundances {scaled} F1 131C Sample']].copy(),
                dtype=float),
            y=np.array(
                cm_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 {cm_dict['130C']} Sample',
                                                           f'Abundances {scaled} F1 {cm_dict['131N']} Sample',
                                                           f'Abundances {scaled} F1 {cm_dict['131C']} Sample']].copy(),
                dtype=float),
            alternative='greater')
    ...
    ################################################################################
    print(
        f'Test differences in {scaled} expression of unique peptides for CM Hypo vs DMSO')
    wilcoxon_signed_test_ps = []
    for unique_peptide in cm_unique_peptdies_df.index:

        wilcox_signed_result = stats.wilcoxon(
            x=np.array(
                cm_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 127N Sample',
                                                           f'Abundances {scaled} F1 128N Sample',
                                                           f'Abundances {scaled} F1 129N Sample']].copy(),
                dtype=float),
            y=np.array(
                cm_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 {cm_dict['127N']} Sample',
                                                           f'Abundances {scaled} F1 {cm_dict['128N']} Sample',
                                                           f'Abundances {scaled} F1 {cm_dict['129N']} Sample']].copy(),
                dtype=float),
            alternative='greater')
        # print(wilcox_signed_result)
        wilcoxon_signed_test_ps.append(wilcox_signed_result.pvalue)
    ...
    ################################################################################
    print(
        f'Test differences in  {scaled} expression of unique peptides for HUVEC Hypo vs DMSO')
    wilcoxon_signed_test_ps = []
    for unique_peptide in huvec_unique_peptdies_df.index:

        wilcox_signed_result = stats.wilcoxon(
            x=np.array(
                huvec_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 127 Sample',
                                                              f'Abundances {scaled} F1 129 Sample',
                                                              f'Abundances {scaled} F1 131 Sample']].copy(),
                dtype=float),
            y=np.array(
                huvec_unique_peptdies_df.loc[unique_peptide, [f'Abundances {scaled} F1 {huvec_dict['127']} Sample',
                                                              f'Abundances {scaled} F1 {huvec_dict['129']} Sample',
                                                              f'Abundances {scaled} F1 {huvec_dict['131']} Sample']].copy(),
                dtype=float),
            alternative='greater')
        # print(wilcox_signed_result)
        wilcoxon_signed_test_ps.append(wilcox_signed_result.pvalue)
...
if __name__ == '__main__':
    ...
    main(cm_unique_peps_csv, cm_dict, huvec_unique_peps_csv, huvec_dict, scaled)
